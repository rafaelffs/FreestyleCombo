...
- [~] **Apple Developer** (account owner): Services ID `com.rafaelffs.freestyleCombo.web` created (for the web flow). **Still needed**: confirm "Sign in with Apple" is enabled on the Services ID itself with Domains (`www.fscombo.com`) and Return URLs (`https://www.fscombo.com/login`) configured — the identifier existing isn't the same as the capability being configured on it. Also confirm the **Sign in with Apple** capability is enabled on the main App ID `com.rafaelffs.freestyleCombo` (not shown in the Identifiers screenshot reviewed so far — only the Services ID was visible).
- [x] **Backend config** — real Google web + iOS client IDs and Apple audiences (`com.rafaelffs.freestyleCombo,com.rafaelffs.freestyleCombo.web`) set in `docker-compose.yml` (local). Verified both fail closed correctly (real JWT parsing attempted, not the empty-audiences short-circuit). **Production secrets not yet updated** — still needs to be set wherever `JwtSettings__Secret` etc. are currently set for the deployed API (see `DeploymentPlan/DEPLOYMENT.md`) before this works against `www.fscombo.com`.
- [x] **Web config** — `web/.env.local` (gitignored) created with both the real Google web client ID and the Apple Services ID as `VITE_GOOGLE_CLIENT_ID`/`VITE_APPLE_CLIENT_ID`. Verified via the dev server that Vite actually injects both into the served page. Production web env vars not yet updated.
...
- [ ] **Verify web**: run `npm run dev`, load `/login`, confirm both buttons render (both client IDs are now set) and complete a real sign-in for a brand-new Google account (check a new `AppUser` row was created with the right `AuthProvider`/`ExternalSubject`/auto-generated username) and a returning one. Apple's popup flow may still fail until the Services ID's own "Sign in with Apple" capability + domain/return-URL config is confirmed (see the Apple Developer checklist item above).
