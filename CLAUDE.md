...
#### Share to Social Media (mobile only)

A second option alongside "Share Link" on the combo detail screen's share button (`combo_detail_screen.dart`'s `_openShareOptions` now opens `share_options_sheet.dart`'s choice sheet instead of calling `Share.share` directly) — listed first, labeled "Share to Social Media". Generates a **full Story-canvas-sized transparent PNG** (not a small floating card) — the combo name/stats sit in a bottom text band over a black-to-transparent scrim; everywhere else stays transparent so the person's own photo/video shows through once they add the saved image to their own Story/post. The picker sheet's single action is **Save to device** — captures the PNG and writes it to the photo library via the `gal` package (`InstagramShareService.saveImage`, `Gal.putImageBytes`). Requires `NSPhotoLibraryAddUsageDescription` in `ios/Runner/Info.plist` (add-only photo access, not full library read); a `GalException` with `type == GalExceptionType.accessDenied` surfaces as "Allow Photos access to save the image."

**Direct-to-Instagram handoff was removed.** The original design also handed the PNG straight to Instagram's Story composer via the `instagram-stories://` sticker-share mechanism (`UIPasteboard` + a platform channel), matching the existing `WatchBridge.swift` native-bridge pattern. Confirmed via real device testing that this doesn't work the way the design assumed: `instagram-stories://share` opens Instagram directly into an edit screen with the sticker already placed on a **blank black canvas** — there's no reachable camera/gallery picker from that screen, so the user can't layer the overlay onto their own video the way the whole design was premised on. This is a real, documented limitation of Instagram's sticker-share API (other apps using it — Spotify Wrapped, Strava, Duolingo — hit the same wall): the API expects the *sharing app* to supply the background media too, which we can't do since the user's own freestyle video lives in their camera roll, entirely outside this app. `InstagramShareBridge.swift` (and its `AppDelegate.swift`/`Info.plist`/`project.pbxproj` registration) were deleted rather than kept as dead code — Save to device + manually building the Story in Instagram (adding the saved image as a photo sticker) is the only path that actually works for combining the overlay with a separately-shot video.

Three layouts (`InstagramOverlayStyle`: Minimal/Sequence/Stat) share five independent display toggles (`InstagramOverlayToggles`: name/difficulty/quantity/rating/sequence — `mobile/lib/core/models/instagram_overlay_content.dart`) and a four-way text-size multiplier (`InstagramTextSize`: smallest/small/medium/large, applied to every overlay text element except the fixed-size FSCOMBO wordmark — the wordmark corner mark is a small `BrandMark` (`widgets/brand_mark.dart`, see "App icon / brand rebrand" below; white mark, lime bolt) next to a small "FSCOMBO" label, replacing the plain lime-dot-plus-text treatment it had before the rebrand). Defaults: name/sequence on, difficulty/quantity/rating off (difficulty and quantity/"Trick count" both defaulted on originally; changed to off after manual testing showed a quieter default overlay read better). A nameless combo (same fallback convention as the rest of the app — see "Tricks API" above) always shows its trick sequence instead of a title, and the Name toggle becomes non-interactive since there's nothing to hide. The chip list shows the entire trick sequence up to 20 tricks before collapsing to a "+N more" summary — deliberately generous, since the app's own random-unset-field default range only goes up to 20 (see "Unset-field resolution during generation" above).

Each chip also carries a weak-foot marker, computed in `overlayChips` (`_trickAbbreviations` returns `({String label, bool? strongFoot})` records — null `strongFoot` for a transition trick, same suppression as every other foot/no-touch marker in the app) and rendered by `_ChipRow` (`instagram_overlay.dart`) via `Text.rich`: a weak-foot chip gets a small, muted `" wf"` suffix (`fontSize: 5.5 * scale`, `alpha: 0.6`, vs. the label's own `7.5 * scale` full-white — no parentheses, unlike the `(wf)`/`·wf` convention elsewhere in the app), so it doesn't compete visually with the trick label. Strong foot (the common case) and transition chips (e.g. "Combo") get no suffix at all. Went through a few iterations here: first a same-size `(wf)` plus a small muted `sf` on every strong-foot chip, then dropping `sf` entirely and keeping `(wf)` at full weight, before landing on this — a small `wf`, no parens, matching how `sf` looked when it still existed, but only on the chips that actually need calling out. `OverlayChips.shownStrongFoot` is a `List<bool?>` parallel to `shown` carrying this per-chip.

The bottom/top scrim (`instagram_overlay.dart`) is a 3-stop `LinearGradient` (`Colors.black` at 0.28 alpha held flat, then fading to 0) sized off the **actual measured height of the content**, not a fixed fraction of the canvas — a fixed fraction (tried first: a plain 2-stop 0.8→0 fade over a `0.44`-tall band, then a flat-then-fade version over a fixed `0.3`-tall band) either washed out video backgrounds with visible grey across nearly half the canvas when content was short, or didn't grow to cover taller content (Large text size, more chip rows) so text could sit above full-black coverage. `InstagramOverlay` (`StatefulWidget`, `_InstagramOverlayState`) measures two things via `_MeasureSize` (a small helper that reports `context.size` via `addPostFrameCallback`, changing nothing about layout): `_measuredHeight`, the whole content+wordmark `Column`'s height, and `_measuredContentOnlyHeight`, just the text/chips widget inside `contentPadding` (no wordmark, no padding). Both fall back to estimates for the one frame before the first measurement lands.

Each build computes: `solidHeight = measuredColumn - _kContentLeadingGap` (the flat/max-weight zone) and `fadeHeight = _kContentLeadingGap + contentOnlyHeight * 0.2` (the fade-out zone above it), then `scrimHeight = (solidHeight + fadeHeight).clamp(0.14, 0.55) * kInstagramOverlayHeight` and the gradient's middle stop at `solidHeight / (solidHeight + fadeHeight)`. `_kContentLeadingGap` (`= 20`, the same constant used in `contentPadding`'s own `EdgeInsets.fromLTRB`) is the key fix in this formula: an earlier version put the *whole* measured column into `solidHeight`, which included that 20px gap `contentPadding` puts between the content and the far (non-edge) side of the column — full-opacity coverage extended over that gap even though nothing was drawn there, which read as a wide flat "empty" grey band sitting well above the actual first line of text (confirmed by exporting a real PNG via Save to device and compositing it over white — `python3`/Pillow, alpha-sampled down the scrim's edge — rather than trusting how it looked against the picker's own purple preview background, which is far more forgiving of grey than a plain white photo). Moving that gap into `fadeHeight` instead means the flat zone now covers only the wordmark + real text, and the fade begins right where the content starts — "closer to the first text," per the reported issue. The alpha itself (0.28, down from an initial 0.78 through 0.5 and 0.38) was mostly chasing the same root cause — it looked "too grey"/"too strong" largely *because* of that oversized flat zone, not just the alpha value in isolation — so don't assume alpha needs to go lower still without re-checking a real exported PNG against white first. Verified across Minimal/Smallest through Sequence/Large in the iOS Simulator, and via direct pixel-sampling of the actual exported PNG (not just the on-screen preview).

The text band can anchor to either edge of the canvas (`InstagramOverlayPosition`: top/bottom, default bottom) via a POSITION chip row in the picker sheet — flips the scrim's gradient direction and the content/wordmark Column order so the wordmark always stays closest to whichever edge the band is on. The wordmark is laid out as the *last* item in the same Column as the content (not an independently-`Positioned` Stack sibling) specifically so Flutter's layout guarantees it can never visually collide with a long chip row — an earlier version positioned them independently and a real device screenshot showed the wordmark overlapping the last trick chip.

When the band is at the top, the internal order of each layout's own pieces (title/name, meta line, chips) is also reversed (`_stackPieces(..., reversed: isTop)` in `instagram_overlay.dart`) — so the combo name ends up closest to the photo/video (bottom of the top band) and the trick chips end up closest to the true top edge, mirroring how it reads when the band is at the bottom rather than just relocating the same top-to-bottom order to a different edge. The wordmark also gets extra top clearance (34 vs. the normal 8) specifically in top position, since Instagram's own Story UI (profile picture, username, close button) sits along the true top edge on top of everything, including this sticker, and would otherwise overlap it.

Picker sheet section order is Layout → Text Size → Position → Show on overlay (Text Size deliberately grouped right under Layout, both being "how the overlay looks" controls, ahead of the display toggles). `_chipRow` (shared by the Layout/Text Size/Position rows) wraps a plain `Row` of naturally-sized `ChoiceChip`s in a horizontal `SingleChildScrollView`, so a row that doesn't fit (e.g. Text Size's four chips, "Smallest" being the longest label) scrolls sideways instead of either breaking to a second line (the original `Wrap` did this) or forcing chips into equal-width `Expanded` slots (tried once, reverted — different-length labels centered in equal slots produced uneven gaps between them, an uglier result than either). The picker's `ListView` also sets `physics: ClampingScrollPhysics()` — without it, iOS's default overscroll bounce at either end of the list fed drag deltas into the enclosing `DraggableScrollableSheet`'s own resize handling, so scrolling past the top or bottom of the sheet's content closed the whole modal instead of just bouncing in place.

The same `InstagramOverlay` widget instance renders both the live on-screen preview (`instagram_share_sheet.dart`) and the actual export — `RenderRepaintBoundary.toImage(pixelRatio: 1080/220)` rasterizes the fixed 220×391-logical-pixel canvas to ~1080×1920 real pixels, entirely client-side (no server round-trip, works offline). File/class names (`instagram_share_sheet.dart`, `InstagramShareService`, etc.) still say "Instagram" — kept as-is since renaming would be a large, low-value diff; only user-facing copy changed. See `docs/superpowers/specs/2026-09-08-instagram-share-design.md` for the original design rationale (its "Instagram handoff plumbing" open question is the one that turned out not to work as assumed).

**Mobile only, not wired up on web.** Now that the feature is just "render an overlay, capture it, save to the photo library" (no platform-specific pasteboard/URL-scheme handoff left), there's no inherent iOS-only blocker anymore — `gal` supports Android too — but no Android or web entry point has actually been built or tested; this app has only shipped iOS so far (see "Production Deployment" below). Extending to Android/web would be a follow-up, not yet started.

**Building an ad-hoc share image, not tied to any saved combo.** `combos_screen.dart` and `tricks_screen.dart` both have an `Icons.ios_share` button in the AppBar, between refresh and add (visible to everyone, logged in or not, since nothing here touches the account) — same gradient tile styling as the add button next to it (`gradient: true` on `_AppBarIconButton`/`_CircleIconButton`), matching the app's existing "primary action" visual language rather than reading as a secondary/neutral icon. Tapping it calls `showInstagramShareSheet(context)` with no combo. The existing per-combo entry points (`combo_card.dart`/`combo_detail_screen.dart`'s share button → `share_options_sheet.dart` → "Share to Social Media") are unaffected and still pass a real combo straight through.

**Trick-list editing is a separate full-screen route, not an inline section of the Share Image sheet.** The first version embedded a `ComboSlotEditor` (name field, picker, reorderable sequence) directly in the sheet below the layout/style controls — cramped next to the preview image, and (see the bug below) the reason "Save to device" started failing. Replaced with `showComboSlotEditorScreen(context, {initialName, initialSlots})` (`instagram_share/combo_slot_editor.dart`) — a `Navigator.push`ed `ComboSlotEditorScreen` (full `Scaffold`, AppBar "Select Tricks", back arrow = cancel) with the *entire* screen for the name field, the "Add tricks"/"Sequence (N)" two-segment control, the picker (search + All/Tricks/Combos filter, exact-match-boosted), and the reorderable slot list — same `SlotItem`/`SlotTile`/`SubComboSlotTile`/`renumberSlots` from `widgets/combo_slot_tile.dart` as `create_combo_screen.dart`'s build tab, but a **fresh, self-contained implementation** rather than an extraction from that screen (matches this codebase's documented per-screen-private-widget convention — see `_OptionalField` elsewhere in this file — and avoids regression risk to the already-complex, well-tested build/generate screen). A bottom "Done" button pops the route with a synthetic, never-persisted `ComboDto` (blank id/ownerId/createdAt, zeroed ratings, difficulty/trick-count computed from the trick pool same as `create_combo_screen.dart`'s `_totalDiff`/`_totalTrickCount`); backing out via the AppBar's back arrow pops `null`.

Both entry points now go through this same route:
- **New toolbar icon** (`combos_screen.dart`/`tricks_screen.dart`, no existing combo): `showInstagramShareSheet` pushes the editor screen *first* — there's nothing to preview until the user actually builds something. A `null` result (backed out without adding anything) means the Share Image sheet never opens at all; a built combo opens it pre-filled.
- **Existing combo** (`share_options_sheet.dart` → "Share to Social Media"): opens the Share Image sheet directly with the real combo, as before. Its "SELECT COMBO/TRICK" section is now just a tappable summary row ("N tricks selected — tap to edit") that pushes the *same* editor screen pre-filled from the current `_liveCombo`, and swaps `_liveCombo` for the result on return (a `null` result — user backed out — leaves it unchanged).

Since every combo the Share Image sheet ever receives is now non-null and non-empty by construction, `_InstagramShareSheet.combo` is a required non-nullable `ComboDto` — the earlier `_emptyCombo` sentinel is gone.

**A real image-capture bug surfaced while the trick editor still lived inline (now history, but worth knowing the shape of it if the sheet's content ever grows tall again).** With the editor inside the same `ListView` as the preview, scrolling down to use it moved the preview's `RepaintBoundary` off-screen — "Save to device" then failed, first with a null `boundaryKey.currentContext` (widget unmounted; a large `ListView.cacheExtent` fixed that specific symptom), then with `AssertionError('!debugNeedsPaint')` inside `RenderRepaintBoundary.toImage()` instead. Instrumenting the render pipeline confirmed this happened on every attempt, indefinitely, with **zero** relayout in between — ruling out the overlay's own self-measuring `_MeasureSize` widgets settling late, the Save button's spinner rebuild racing the capture, and sub-pixel float jitter re-triggering `setState` forever (none of these were actually occurring). The boundary was cleanly settled and simply never repainted, because cache-extent keeps off-screen sliver children mounted and laid out — not necessarily *painted* — once they're far enough outside the viewport.

The fix at the time (pinning the drag handle + title + preview outside the scrollable settings list, in a fixed `Column` before an `Expanded(child: ListView(...))`) *did* solve that capture bug, but broke something else: a `GestureDetector(onVerticalDragUpdate: (_) {})` had to wrap the now-unscrollable header to stop a drag there falling through to the modal route's own whole-sheet drag-to-dismiss — and that absorber ate the gesture entirely, so a swipe *starting* anywhere over the image (the most natural place to put a thumb) did nothing at all, neither scrolling nor dismissing. **Since the trick editor moved to its own full-screen route (above), this whole pin was reverted** — `_InstagramShareSheetState.build()` is back to one plain `ListView` with the preview as its first item, no pinning, no gesture absorber. The settings list is short again (Layout/Text Size/Position/Show on overlay/Select combo/trick summary row/error/Save button — no embedded picker) so the preview never scrolls far enough off-screen to reopen the original capture bug, and the sheet's drag-to-scroll/drag-to-dismiss behaves normally everywhere, image included. If this list ever grows tall again, don't reach for the pin-with-absorber pattern — it trades one real bug for a worse one; a pinned *sliver* header (`SliverPersistentHeader(pinned: true)` inside a `CustomScrollView`) would keep the preview visible without removing it from the scroll gesture, unlike a `Column`-outside-`ListView` split.

`InstagramShareService.capturePng`/`saveBytes` (split from the old combined `saveImage`) still keeps a small bounded retry around `AssertionError` as a fallback for a genuinely transient one-frame-stale capture, and `_saveImage()` still captures *before* calling `setState(() => _saving = true)` so the button's own spinner rebuild can't race the capture — both harmless, defensive, and orthogonal to the pin/no-pin question above.
...
### Combo name display mode
A 3-way mode, not a boolean — went through a boolean "Show combo name" toggle first (hide-entirely-when-off), then this replaced it once a middle ground was requested: **Show** (default — `combo.name` when set, else the displayText/sequence fallback, same as the app's original unconditional behavior), **Hide unnamed** (show a real name, but render nothing at all for a combo that doesn't have one — no fallback title), **Hide always** (never render the title slot, regardless of whether the combo has a name).

Web: `displayPrefs.ts`'s `ComboNameDisplayMode` (`'show' | 'hideUnnamed' | 'hideAlways'`, `localStorage` key `fc_combo_name_display`, default `'show'`) plus the pure helper `comboTitleUsesName(hasName, mode): boolean | null` (true = use `combo.name`, false = use the fallback, null = render nothing) that both `ComboCard` and the `<select>` control on `CombosPage` (`combos.nameDisplayLabel`/`nameDisplayShow`/`nameDisplayHideUnnamed`/`nameDisplayHideAlways` i18n keys, replacing the old checkbox) key off. `ComboDetailPage`'s hero is the one asymmetric case: it only ever gates the small `combo.name` line (`combo.name && getComboNameDisplay() !== 'hideAlways'`) — `CardTitle` (`combo.displayText`, mono) is a separate, always-shown secondary line there, unaffected by any mode, matching the page's original "show only the abbr" design; `hideUnnamed` and `show` behave identically on this page since the name line only ever renders when a name already exists.

Mobile mirrors this as `ComboNameDisplayMode` enum + `ComboNameDisplay.mode` (`mobile/lib/widgets/combo_card.dart`, same static-flag pattern as `TrickNameDisplay`) plus the equivalent `ComboNameDisplay.useName(hasName): bool?` helper, used by both `combo_card.dart`'s title and `combo_detail_screen.dart`'s hero title. Deliberately not wired into `ComboItem.displayName` (`mobile/lib/core/models/combo.dart`) — that getter also drives sub-combo search/matching in the build/edit trick pickers, where the name has to stay a stable identifier regardless of this display-only setting.

The picker UI (`mobile/lib/widgets/display_options.dart`, `_ComboNameDropdown`) went through two layouts before landing on a `PopupMenuButton`-based dropdown: first a labeled 3-chip `SegmentButton` row (dropped — a three-way row read as three separate toggles rather than one selector, and "Hide unnamed" was long enough to need horizontal scrolling next to a label, which read awkwardly for a settings row), then a `DropdownButton` styled to look like a small pill (dropped — it silently ate every tap in this settings-sheet context specifically; every other control in the same sheet, including a `PopupMenuButton` elsewhere in the app, responded fine, isolating the problem to `DropdownButton` itself here rather than the sheet). The current version wraps a `PopupMenuButton<ComboNameDisplayMode>` around a small `Container` (chip-styled, current label + a chevron `Icon`) as its `child`, `onSelected` writing straight to `ComboNameDisplay.mode` — same widget already proven working for row actions in `admin_users_screen.dart`.
...
**Live at `https://www.fscombo.com`** — API + web are in production (see `DeploymentPlan/DEPLOYMENT.md` for the original rollout plan). **`1.0.0` through `1.0.3` are all live on the App Store** (`appStoreState=READY_FOR_SALE` for all four, confirmed 2026-09-16 — `1.0.3` shipped sometime between 2026-09-14 and 2026-09-16, no explicit "it shipped" checkpoint was caught in between; if a future session needs the exact approval date, check App Store Connect's version history directly). A `1.0.4` version was created 2026-09-16 with build 26 attached (`fastlane ios create_and_attach_version app_version:1.0.4 build_number:26`), release notes synced to both locales, screenshots (including the required Watch one) confirmed `COMPLETE` for both locales without needing re-upload, then submitted for review via the manual "Submit for Review" click in App Store Connect (the API itself still refuses to create the submission for this app, same as every previous version) — confirmed `appStoreState=WAITING_FOR_REVIEW` with a valid `AppStoreVersionSubmission` object, `releaseType=AFTER_APPROVAL` so it auto-releases once approved, no further action needed. `1.0.4` bundles everything merged since `1.0.3+23`: the "Around The World" brand rebrand (app icon, favicon, in-app `BrandMark` everywhere the old soccer-ball icon appeared), and the ad-hoc share-image feature (new gradient share icon on Combos/Tricks pages, full-screen trick-list editor separate from the Share Image sheet, plus the scroll/capture bugs found and fixed along the way). See "Checking App Store submission status" below for how to re-check state in a future session.
...
- **App Store prerequisites now in place**: production backend reachable over HTTPS, `/privacy` and `/terms` pages live, self-service account deletion (`DELETE /api/account/me`, wired into both `AccountPage` (web) and `account_screen.dart` (mobile)) per Apple Guideline 5.1.1(v), real branded app icon (not a placeholder — see "App icon" below) in `ios/Runner/Assets.xcassets/AppIcon.appiconset/`. Apple Developer Program membership is active and paid — Team ID `6K8AXR83Y3` is set (`DEVELOPMENT_TEAM` in `ios/Runner.xcodeproj`, `CODE_SIGN_STYLE = Manual`, written by fastlane's `update_code_signing_settings`). Bundle ID `com.rafaelffs.freestyleCombo` is registered and the App Store Connect app record exists (app id `6806225332`). A demo/review account is seeded on production (`applereview` / see 1Password or ask the account owner — verified live via `/api/auth/login`). TestFlight has an "Internal Testers" group; builds 1–26 are all `VALID` and export-compliance-cleared. Build 6 (`1.0.0+6`) was the first to include Google/Apple sign-in; build 7 (`1.0.0+7`) added the raw-exception-dump fix; build 8 (`1.0.0+8`) replaces the placeholder app icon (see below); build 9 (`1.0.0+9`) adds combo link sharing (needed a `fastlane ios setup_signing` re-run first — its new `com.apple.developer.associated-domains` entitlement wasn't in the provisioning profile yet) plus a round of mobile UX polish; builds 10–14 (`1.0.0+10` through `+14`) shipped the Apple Watch companion app incrementally plus various mobile fixes; build 15 (`1.0.1+15`) was the first attempt at shipping the optional-preference-fields work but was superseded before submission; build 16 (`1.0.1+16`) shipped as `1.0.1`; builds 17–21 (`1.0.2+17` through `+21`) shipped as `1.0.2` (Share to Social Media, Watch offline support — see git log on those build-bump commits for specifics, not itemized here); build 22 (`1.0.3+22`) was a mid-iteration upload later superseded; build 23 (`1.0.3+23`) shipped as `1.0.3`; build 24 (`1.0.3+24`) is the "Around The World" brand rebrand — TestFlight-only, never got its own App Store version; build 25 (`1.0.4+25`) is the ad-hoc share-image feature (new share icon on Combos/Tricks, full-screen trick editor); build 26 (`1.0.4+26`, 2026-09-16) is a follow-up polish fix (the Select Tricks screen's combo-name field now matches the build screen's white/bordered styling instead of a greyed chip fill) — build 26 is the one attached to the pending `1.0.4` App Store version (see above). Build 25's first upload attempt at marketing version `1.0.3` was rejected by `upload_to_testflight` ("CFBundleShortVersionString [1.0.3] ... must contain a higher version than that of the previously approved version [1.0.3]") because `1.0.3` had gone `READY_FOR_SALE` since the build-24 check — bumping to `1.0.4` (build number unchanged) fixed it, same gotcha #6 below. New builds are uploaded via `fastlane ios beta`, then need `fastlane ios clear_export_compliance` before they're usable in TestFlight (recent builds — including 22 through 26 — have come back from Apple's processing already compliance-cleared automatically, but don't assume that — check first). App Store screenshots (6.9" class, 1320×2868, branded) are in `design/appstore-screenshots/marketing/` — unchanged since `1.0.0`, not re-uploaded since.

**App icon / brand rebrand (2026-09-15)**: the identity changed from the original purple-gradient tile + white/purple soccer-ball pattern to an **"Around The World" (ATW) mark** — an orbit ring (one ellipse, rotated −28°) with a solid ball at the centre (a lightning bolt cut into it, standing for the app's Generate action) and a small "travelling" ball riding the ring. Brand gradient is now 3-stop `#5B4FE9 → #7A5AF0 (55%) → #8E6BF5`, 135°; solid indigo `#4F46E5`; ink `#15131F`. Source of truth is **`design/logo-kit/`** (added to the repo this pass — mark/, lockup/, app-icon/, favicon/ SVGs+PNGs, plus its own `README.md` with clear-space/minimum-size/usage rules) — regenerate everything below from there, don't hand-edit rasters or re-derive coordinates from scratch.

Touchpoints updated in this pass, all re-derived from the same `logo-kit` source so they can't drift out of sync with each other again the way the pre-rebrand assets had (the old Watch app icon and `og-image.png` were already both subtly different renders of the old soccer-ball pattern from `favicon.svg`'s own coordinates — a pre-existing inconsistency, moot now that everything shares one source):
- `web/public/favicon.svg` — kit's `favicon/favicon-32.svg` content (rounded-rect gradient tile, higher ring opacity than the bare mark so it survives small), stripped to `viewBox`-only (no fixed `width`/`height`) so it scales to whatever a browser requests, matching the pre-rebrand file's own convention.
- `web/public/apple-touch-icon.png` (new — index.html previously pointed `apple-touch-icon` at the SVG, which iOS honors inconsistently) — 180×180 PNG, full-bleed, no pre-rounding, no alpha channel, same convention as the iOS home-screen icons below.
- `web/index.html` / `web/public/site.webmanifest` — `theme-color`/`theme_color` updated `#5B4AD4` → `#4F46E5`; manifest gained `icon-192.png`/`icon-512.png` (both new, `maskable`) alongside the existing scalable `favicon.svg` entry.
- `web/public/og-image.svg`/`.png` — only the icon slot and the two accent text colours were touched (gradient tokens + `#4F46E5`/`#15131F`); the "FREESTYLE"/"Combo" text itself, its size, and its position are untouched. The `.png` was regenerated by compositing a separately-rendered icon tile onto the *existing* PNG at the same slot rather than re-rastering the whole 1200×630 SVG in one pass — QuickLook's SVG thumbnailer (`qlmanage`, see below) doesn't reproduce the `Helvetica Neue`/`Arial` fallback metrics faithfully, and a full re-render clipped "Combo" at the canvas edge.
- `web/src/components/Logo.tsx` — the old `Football`/`AppIcon` components (parametric pentagon-patch math, "a similar but not pixel-identical pattern" to the old favicon per the previous note here) are gone; `AppIcon` now hand-codes the exact same four shapes as `logo-kit`'s app-icon SVGs (ellipse/circle/path/circle, `viewBox="0 0 100 100"`) inline, still with no external asset request. Wordmark updated to the kit's exact spec: `FREESTYLE` 800 weight (was 600) indigo `#4F46E5`/violet `#9C8BFF` on dark, `Combo` letter-spacing `-0.9` (was `-0.5`) ink `#15131F`.
- `mobile/ios/Runner/Assets.xcassets/AppIcon.appiconset/` (all 15 files) and `mobile/ios/Watch App Watch App/Assets.xcassets/AppIcon.appiconset/AppIcon-watch-1024.png` (the Watch app has its own separate icon set, previously undocumented here) — regenerated from one flattened 1024 master.
- `mobile/ios/Runner/Assets.xcassets/LaunchImage.imageset/` — was still Flutter's stock 1×1 fully-transparent placeholder pixel at all three scales (never actually customized, unlike the app icon which at least had a wrong-but-real image before build 8) — now the bare gradient mark (`logo-kit/mark/logo-mark-gradient.svg`, no background tile — it's already sitting on the storyboard's white background) centred at 160/320/480pt.
- `mobile/lib/widgets/brand_mark.dart` (new) — `BrandMark`, a `CustomPainter` that hand-draws the same four shapes as `logo-kit/mark/logo-mark-*.svg` (orbit ellipse, ball, bolt, satellite ball) at any size/colour, no `flutter_svg` dependency needed. Replaces the generic `Icons.sports_soccer` Material icon that both the auth-screen gradient hero (`auth_chrome.dart`, white mark + ink bolt in the 66×66 hero tile) and the bottom-nav Combos tab (`main_shell.dart`, indigo mark + white bolt, replacing the old `sports_soccer_outlined` icon — not the center Generate FAB, which stays a plain lightning-bolt icon since it represents the Generate action, not the brand) were still drawing after the initial rebrand pass — fixed per explicit follow-up ("i said all. even mobile icons"). Verified visually in the iOS Simulator (both usages render correctly) and via `flutter analyze`/`flutter test` (35/35 passing).
- **Apple Watch app**: checked its SwiftUI source for any brand-representing icon (`Image(`/`systemName:` across all Swift files) — none found; its only brand touchpoint is the watch app icon itself (`AppIcon-watch-1024.png`, already covered above).

**Rasterization approach** — no SVG rasterizer (`rsvg-convert`, `resvg`, `cairosvg`) was available on the machine, same constraint as the original icon fix, but this time `qlmanage -t -s <pixels> -o <dir> <file>.svg` (macOS's built-in Quick Look thumbnailer, no install needed) worked as a stand-in, with two gotchas worth knowing before reaching for it again:
1. **It flattens all SVG transparency to opaque white.** Fine when the target background is white anyway (the launch image); wrong for anything that needs real alpha (the rounded-corner app-icon tiles, which must stay full-bleed and un-rounded regardless — see below — so this didn't end up mattering) or that gets composited onto a non-white surface (the `og-image.png` icon tile, where it showed up as a visible white square) — for that case a PIL-drawn `ImageDraw.rounded_rectangle` alpha mask (supersampled 4× then downsampled for anti-aliasing) was used instead of trusting the SVG's own transparent corners.
2. **It does not rescale content whose SVG declares a `width`/`height` smaller than the requested `-s` size** — it renders at the SVG's own native declared size and pads the rest of the canvas, anchored top-left, rather than scaling the vector content up to fill it (this bit the launch-image mark, originally declared at `100×100`, rendered at `-s 720`). Always either strip `width`/`height` entirely (letting `viewBox` drive scaling to whatever size is requested — what `favicon.svg` does) or rewrite them to exactly match the `-s` value passed (what every fixed-size render here does) before rasterizing.

For the iOS/watchOS home-screen icons specifically: full-bleed square, **no manual corner rounding** (iOS/watchOS apply their own mask; the kit's own `app-icon-1024.svg` source ships with `rx="240"` baked in for on-canvas previews, which had to be stripped back to `rx="0"` before rasterizing), and **no alpha channel** (`.convert('RGB')` in Pillow, not just alpha=255 — the 1024 App Store marketing icon is rejected outright if it merely *looks* opaque but still carries an alpha channel). If any of this needs regenerating again, start from `design/logo-kit/app-icon/app-icon-1024.svg`, not from a previous PNG.
...
- The `·nt`/`·wf` suffix is built independently for each flag (`'${t.noTouch ? '·nt' : ''}${!t.strongFoot ? '·wf' : ''}'`) everywhere it's a concatenated string — `combo_card.dart`'s `_buildChip`, `combo_detail_screen.dart`'s sub-combo trick-chip suffix, `admin_submissions_screen.dart`, `tricks_screen.dart`'s reusable-combo expansion, `combo_slot_tile.dart`'s sub-combo expansion. All five used to build it as `noTouch ? '·nt' : (!strongFoot ? '·wf' : '')` — an if/else, not independent conditions — so a trick that was both no-touch *and* weak-foot silently dropped the `·wf` half and showed only `·nt`. Web never had this bug: its nt/wf markers are always two independent sibling `<span>`s, never concatenated into one ternary.
...
**Nine real release-pipeline bugs/gotchas already found and fixed — don't rediscover these:**
...
9. Gotcha #1's fix (prepending Homebrew Ruby to `PATH`) stopped being sufficient once a Homebrew upgrade moved fastlane itself onto Ruby 4.0 (fastlane's own gem bin at `~/.local/share/fastlane/4.0.0`, ahead of `/opt/homebrew/opt/ruby/bin` on `PATH` inside every `sh()` call) — fastlane's own Ruby process leaks its `GEM_PATH` (`~/.local/share/fastlane/4.0.0:...:/opt/homebrew/Cellar/cocoapods/1.17.0/libexec`) into any `pod`/`ruby` subprocess spawned via `sh()`, and `pod`'s own vendored `GEM_HOME` (cocoapods is installed as a standalone Homebrew formula with its own `libexec`, not a regular gem) then collides with that leaked `GEM_PATH` on an `activesupport` version mismatch — `pod --version` throws a `Gem::Specification#activate_dependencies` error under `sh()`, even though running the exact same `pod --version` directly in a plain shell works fine, which is what makes this look identical to gotcha #1's stale-PATH symptom (same "CocoaPods not installed or not in valid state" message) while actually being a different root cause. Confirmed the fix from gotcha #1 does not fully solve this: it only covers `ruby`/`ENV["PATH"]`, not the `GEM_PATH` fastlane's own process already has set before the Fastfile even runs. **Fixed permanently** in the `build` lane: `sh("unset GEM_HOME GEM_PATH BUNDLE_GEMFILE; flutter build ipa ...")` clears the leaked variables for just that one subprocess, right before the `flutter build ipa` invocation — re-verified working by running `fastlane ios build` (not the manual-shell workaround) end-to-end afterward. `fastlane ios beta`/`clear_export_compliance` (pure ASC API calls, no `pod`/`ruby` subprocess) were never affected by this and don't need the same treatment.
