...
**Live at `https://www.fscombo.com`** — API + web are in production (see `DeploymentPlan/DEPLOYMENT.md` for the original rollout plan). Mobile: an App Store submission (`1.0.0`) is in Apple's review queue (`appStoreState=WAITING_FOR_REVIEW` as of 2026-09-01, attached to build 7 — see "Checking App Store submission status" below for how to check current state). TestFlight build 7 (`1.0.0+7`) has Google/Apple sign-in plus the raw-exception-dump fix.

`feature/batch-release` (personal reusable combos, various mobile/web fixes, Google/Apple sign-in, release-pipeline fixes) merged to `main` and deployed on 2026-08-31. `GOOGLE_AUTH_AUDIENCES`/`APPLE_AUTH_AUDIENCES` are set in the VPS's `/opt/freestylecombo/.env` (see Environment variables table) — Google and Apple sign-in confirmed working end-to-end against production via a real TestFlight build.
