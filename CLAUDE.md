...
**Live at `https://www.fscombo.com`** — API + web are in production (see `DeploymentPlan/DEPLOYMENT.md` for the original rollout plan). **`1.0.0` through `1.0.3` are all live on the App Store** (`appStoreState=READY_FOR_SALE` for all four, confirmed 2026-09-16 — `1.0.3` shipped sometime between 2026-09-14 and 2026-09-16, no explicit "it shipped" checkpoint was caught in between; if a future session needs the exact approval date, check App Store Connect's version history directly). `1.0.3+23`'s App Store version bundled everything merged since `1.0.2+21`: the 3-way combo-name display mode (Show/Hide unnamed/Hide always, web + mobile), the Instagram share overlay's scrim reworked to size off actual content instead of a fixed canvas fraction, an in-app entry point added for the mobile admin users list, and removing `Email` from the public-by-id profile endpoint. No `1.0.4` App Store version has been created yet — build 25 (see below) is TestFlight-only so far. See "Checking App Store submission status" below for how to re-check state in a future session.
