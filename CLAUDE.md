...
- Config: `Auth:Google:Audiences` / `Auth:Apple:Audiences` (comma-separated OAuth client IDs / Apple audiences — see Environment variables table). Google web + iOS client IDs are configured (`docker-compose.yml`); **Apple audiences still empty** — Apple sign-in fails closed (403) until the account owner completes the Apple Developer Services ID setup. **Android is deliberately not set up yet** (no Google Cloud Android OAuth client, no `mobile/android/app/google-services.json`) — skipped for now, to be created later when Android release work starts.
- Mobile: `google_sign_in` + `sign_in_with_apple` packages, buttons in `SocialSignInButtons` (`mobile/lib/features/auth/social_sign_in_buttons.dart`), shared by `login_screen.dart` and `register_screen.dart`. Apple button is iOS-only (`Platform.isIOS`). `main.dart` calls `GoogleSignIn.instance.initialize(clientId: ...)` on startup with the iOS client ID, only on iOS (`Platform.isIOS ? _googleIosClientId : null`) — Android gets no clientId since it isn't configured yet, so Google sign-in will not work on Android until that setup lands. `ios/Runner/Info.plist` has the reversed iOS client ID registered as a `CFBundleURLTypes` scheme; `ios/Runner/Runner.entitlements` (Sign in with Apple capability) still needs to be added in Xcode before Apple sign-in works on iOS.
- Web: no new npm packages — Google Identity Services and Apple's JS SDK are loaded via injected `<script>` tags (module-scope promise-cached per script id, so concurrent mounts share one in-flight load rather than racing) in `SocialSignInButtons.tsx` (`web/src/features/auth/`), shared by `LoginPage.tsx`/`RegisterPage.tsx`. Buttons render only when `VITE_GOOGLE_CLIENT_ID`/`VITE_APPLE_CLIENT_ID` are set (`web/.env.example` documents both, `web/.env.local` — gitignored — holds the real Google web client ID; Apple's still unset).
