...
**Live at `https://www.fscombo.com`** — API + web are in production (see `DeploymentPlan/DEPLOYMENT.md` for the original rollout plan). **`1.0.0`, `1.0.1`, and `1.0.2` are all live on the App Store** (`appStoreState=READY_FOR_SALE` for all three, confirmed 2026-09-14). A `1.0.3` version was created 2026-09-14 with build 23 attached and submitted for review the same day (`appStoreState=WAITING_FOR_REVIEW`, valid `AppStoreVersionSubmission` confirmed, `releaseType=AFTER_APPROVAL` so it auto-releases once approved, no further action needed) — the "Submit for Review" click itself still had to happen manually in App Store Connect, since the ASC API refuses to create the submission itself for this app, for a brand-new version exactly as for a resubmission (see "Swapping the in-flight version onto a newer build" / "Creating a brand-new version" below). `1.0.3+23` bundles everything merged since `1.0.2+21`: the 3-way combo-name display mode (Show/Hide unnamed/Hide always, web + mobile), the Instagram share overlay's scrim reworked to size off actual content instead of a fixed canvas fraction (fixing both the washed-out-background complaint and a missing-`wf`-marker bug on chips that were also no-touch), an in-app entry point added for the mobile admin users list (the screen existed but had no way to reach it), and removing `Email` from the public-by-id profile endpoint (it had no `[Authorize]`, so this was a real data leak, not just a UI choice). See "Checking App Store submission status" below for how to re-check state in a future session.
